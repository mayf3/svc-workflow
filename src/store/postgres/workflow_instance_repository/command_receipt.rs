//! Command receipt handling for idempotency.
//!
//! Implements the idempotent command receipt pattern:
...
pub(super) enum ReceiptReplayResult {
...
///
/// Must be called after `try_insert_receipt` returns `None`.
pub(super) async fn replay_existing_receipt(
    tx: &mut Transaction<'_, Postgres>,
    principal_id: Uuid,
    idempotency_key: &str,
    new_request_hash: &str,
) -> Result<ReceiptReplayResult, CreateWorkflowInstanceError> {
    // Lock and read the existing receipt
    ...
    match receipt {
        None => {
            // Race condition: the INSERT returned no rows but the SELECT also finds nothing.
            // This shouldn't happen, but handle defensively.
            Err(CreateWorkflowInstanceError::StorageError(
                "idempotency key vanished between INSERT and SELECT".to_string(),
            ))
        }
        Some((cmd_id, status, stored_hash, resp_status, resp_body)) => {
            match status.as_str() {
                "COMPLETED" => {
                    if stored_hash == new_request_hash {
                        // Exact replay — return stored response
                        Ok(ReceiptReplayResult::CompletedMatch {
                            command_id: cmd_id,
                            response_status: resp_status,
                            response_body: resp_body.unwrap_or(serde_json::Value::Null),
                        })
                    } else {
                        // Hash mismatch — idempotency conflict
                        Ok(ReceiptReplayResult::CompletedConflict {
                            command_id: cmd_id,
                            original_request_hash: stored_hash,
                        })
                    }
                }
                "PROCESSING" => Ok(ReceiptReplayResult::StillProcessing),
                _ => Err(CreateWorkflowInstanceError::StorageError(format!(
