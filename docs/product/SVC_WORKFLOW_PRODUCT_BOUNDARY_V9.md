---
authority_id: SVC_WORKFLOW_PRODUCT_BOUNDARY_V9
status: proposed
...
date: 2026-09-15
revision: r1
change_name: WORKFLOW_INSTANCE_COLLABORATION_AUTHORITY_V0
...
#### OBS-V9-002 — Current durable fact and side-band inventory

The authoritative workflow history at the candidate base consists of the
immutable `WorkflowContextRevision`, `NodeVisit`, `Submission`, and
`WorkflowEvent` fact families plus canonical activation facts. Assistance
V1 is a Visit-scoped, status-carrying side-band state machine whose writes
advance `workflowStateVersion`, append Workflow Events, occupy
`eventSequence`, block normal transitions while open, and are voided on
cancel/archive. `workflow_events` enforces
`event_sequence = new_workflow_state_version` and
`new = old + 1` at the storage level, so no row can exist in that table
without a state-version progression.

#### OBS-V9-003 — Owner ruling closing the product decisions

On 2026-09-15 the repository Owner issued
`OWNER_RULING = ACCEPT_WITH_SIMPLIFICATION` for
`WORKFLOW_INSTANCE_COLLABORATION_V1`, closing PD-1..PD-6 in one act:

```text
independent Instance Collaboration side-band model = YES
reuse Workflow Context = NO
reuse Assistance for ordinary exchange = NO
Collaboration Entry changes workflow_state_version = NO
Collaboration Entry consumes workflow event_sequence = NO
RETURN reason duplicated into Collaboration Entry = NO

PD-1 read  = any Principal holding legal instance visibility
            (DomainOwnerFull | CurrentAssigneeFull | Creator
             | HistoricalParticipantRestricted);
            Historical Participant reads complete Collaboration Entries;
            Collaboration visibility != Context/Submission/Event payload
            visibility expansion.
PD-2 write = Domain Owner | Workflow Creator | Current Assignee
            | Historical Participant; principal enabled; instance not
            archived; no string roles, no future-role guessing, no new
            participant identity model.
PD-3       = TERMINAL-but-not-archived CAN_WRITE;
            CANCELLED-but-not-archived CAN_WRITE;
            do not inherit Assistance's cancelled fail-closed semantics;
            the only hard close point is Archive.
PD-4       = ARCHIVED -> Collaboration READ_ONLY, POST fail closed;
            existing Entries retained forever; V1 has NO edit, NO delete,
            NO void.
PD-5       = REFERENCE_DOES_NOT_GRANT_ACCESS = YES.
PD-6       = kind = REMOVE; V1 has no NOTE/DECISION/WARNING/QUESTION/
            ANSWER taxonomy; an Entry is body + optional reply +
            optional references; the server snapshots
            observed_node_visit_id and observed_workflow_state_version
            at append; clients cannot specify or forge them.
```

The same ruling selects the HTTP sub-resource naming
`collaboration/entries` (never `context-entries`), accepts the unified
two-item-type feed projection, and forbids duplicating formal workflow
facts, including any pseudo-success record of failed or rolled-back
RETURNs.

#### CLM-V9-001 — No existing authority covers this semantic

V8 §5.6 closes the authoritative history to the four fact families plus
activation; V8 §6 grants historical participants only "explicitly
governed participation history"; Architecture CTR-ARCH-040 forbids any
child Spec, API, schema, migration, code, or test from adding a concept
absent from the Product Direction and Architecture without a lawful
higher-authority change. An instance-scoped durable collaboration record
therefore requires this Product Direction exception; it cannot be created
at the Architecture or implementation layer.

#### CLM-V9-002 — The side-band shape is composable without touching
state-command semantics

At the candidate base, pure reads and durable no-op outcomes already
coexist with state commands without advancing `workflowStateVersion`, and
the command-receipt idempotency machinery is command-type-generic. A new
append-only fact family bound to the same idempotency and principal
discipline can therefore exist without modifying Transition, Context,
Visit, Event, or Assistance semantics. This is an inference from the
observations above, not an implemented fact.

#### STATE-V9-001 — Candidate state

This document is a proposed whole-authority successor. Nothing in it is
active repository authority, no implementation is authorized, and no
database, wire, or production change may proceed from it before the
governance sequence in §7 completes.

#### DEC-V9-001 — Select the independent side-band collaboration model

Among the three evaluated candidates — extending Workflow Context,
reusing Assistance, or an independent Instance Collaboration side-band
model — the Owner ruling selects the independent side-band model and
rejects the other two. Extending Context would violate create-time
immutable Context and would couple discussion to
`expectedWorkflowStateVersion` conflicts. Reusing Assistance would
pollute a Visit-scoped, version-advancing, transition-blocking help
state machine with ordinary exchange. Both rejections are recorded so
they are not re-litigated at lower layers.

### 2. Selected V0 product semantic

A Workflow Instance owns one durable Collaboration space: an append-only
sequence of Collaboration Entries authored by canonical Principals,
readable through a unified Collaboration Feed projection that merges the
Entries with a minimal, directly projected set of canonical workflow
facts.

A Collaboration Entry is an immutable, kind-free record:

```text
body                          free text, bounded
reply_to_entry_id             optional reference to an earlier Entry of
                              the same Instance
related_event_id              optional same-Instance Workflow Event
related_submission_id         optional same-Instance Submission
related_assistance_case_id    optional same-Instance Assistance Case
observed_node_visit_id        server-authored snapshot of the Instance's
                              current Node Visit at append time
observed_workflow_state_version
                              server-authored snapshot of the Instance's
                              current workflow state version at append
                              time; advisory observation record only,
                              never an optimistic-concurrency predicate
author                        the verified canonical Principal (token.sub
                              provenance); never client-supplied
```

V1 deliberately has no `kind` taxonomy: replies already express
question/answer structure, plain text already expresses reminders, and a
formal workflow Decision must not be counterfeited by a chat enum. If a
strong need appears later, a separately accepted authority adds it.

The product purpose is durable context recovery: a completely fresh Agent
Session that gains a Node Visit on this Instance must be able to recover,
using only svc-workflow durable facts, why prior RETURNs happened, what
the participants clarified, what prior Submissions delivered, and what
remains to be done. "Being able to chat on a web page" is not the
product goal.

### 3. Authorization

Read: any Principal that holds legal visibility on the Workflow Instance
under the existing classification (`DomainOwnerFull`,
`CurrentAssigneeFull`, creator, `HistoricalParticipantRestricted`) may
read the Collaboration space. A Historical Participant reads the complete
Collaboration Entries. Collaboration visibility is not Context,
Submission, or Event payload visibility expansion: referencing a
Submission, Event, or Assistance Case never grants its payload; every
canonical workflow fact keeps its own read authority.

Write: a Principal may append a Collaboration Entry when it holds at
least one of Domain Owner, Workflow Creator, Current Assignee, or
Historical Participant relation to the Instance, the Principal is
enabled, and the Instance is not archived. TERMINAL-but-not-archived and
CANCELLED-but-not-archived Instances remain writable: Collaboration is a
side-band collaboration record, not a workflow execution capability, and
post-terminal explanation, supplementation, handover, retrospective, or
final notes are in scope. Assistance's cancelled fail-closed semantics
are intentionally not inherited.

Archive: an archived Instance's Collaboration space is read-only; append
attempts fail closed; existing Entries are retained forever. V1 has no
edit, no delete, and no void for Collaboration Entries under any
lifecycle state.

Identity: authors are always real canonical Principals bound to verified
authentication identity. No string role names, no future-role guessing,
and no new participant identity model are introduced by this exception.

### 4. Hard non-interference boundary

Creating a Collaboration Entry:

```text
MUST NOT modify Workflow Context or create a Context Revision
MUST NOT execute or influence a Transition
MUST NOT create or modify a Node Visit or activation
MUST NOT change any assignee or current projection
MUST NOT increment workflow_state_version
MUST NOT create a Workflow Event or consume event_sequence
MUST NOT open, escalate, resolve, or void an Assistance Case
MUST NOT wake, dispatch, schedule, or retry anything
```

Conversely, no concurrent Collaboration append may cause a
state-command's `expectedWorkflowStateVersion` check to fail, and no
state command needs to account for Collaboration Entries in its
concurrency control. A comment can never produce a version conflict for
the executing Agent.

### 5. Explicit non-goals and lane boundaries

```text
NO message edit, delete, reaction, or void
NO @mention notification system
NO full-text search
NO WebSocket / realtime chat transport
NO attachment upload
NO cross-instance threads
NO kind taxonomy (NOTE/DECISION/WARNING/QUESTION/ANSWER all rejected in V1)
NO AI-generated summary as authority
NO comment-triggered Agent wake, dispatch, or auto-transition
NO new scheduler, retry, or delivery framework
NO second copy of any formal workflow fact (RETURN reason stays in
   Submission/Event authority; Assistance payloads stay in Assistance
   authority)
NO UI ownership (presentation belongs to UI products)
NO expansion of Context/Submission/Event/Assistance payload visibility
```

Ordinary Collaboration is not Assistance: it never blocks transitions,
never advances state, and never enters the Assistance lifecycle. It is
also not a second scheduler: nothing in the Collaboration space is a
wait primitive, eligibility predicate, or dispatch input.

### 6. Invariant and activation gate

Implementation of this exception may begin only when: this V9 authority
is Owner-accepted and merged through the atomic lifecycle transaction;
the `SVC_WORKFLOW_ARCHITECTURE_V0_4_2` successor is likewise accepted;
and an accepted implementation Spec with `implementation_authority:
contracts` covering this scope is present in the implementation PR base.
Production apply remains a separate gate. Nothing in this candidate
authorizes code, migration, wire change, or deployment by itself.

### 7. Authority sequence and acceptance

```text
1. this docs-only whole-authority candidate (V9) plus the companion
   architecture successor (V0_4_2) and implementation Spec
   (SVC_WORKFLOW_INSTANCE_COLLABORATION_V1), semantically consistent
2. one independent semantic review of the exact candidate heads
3. one Owner exact-head acceptance
4. atomic lifecycle transaction: V9 frontmatter -> accepted (with
   acceptance-transaction fields), V8 frontmatter -> superseded with
   superseded_by backlink, V0_4_1/V0_4_2 likewise, repository-local
   authority map switched, spec acceptance transaction flips
   implementation_authority none -> contracts
5. implementation PR against the accepted bases, Contract-by-Contract
   evidence, separate production gate
```

### 8. V9 normative contracts

#### CTR-V9-001 — Whole-authority lifecycle
V9 MUST remain inactive candidate authority while unmerged. It becomes
the Product Direction only through the completed atomic transition with
V8 backlink and authority-map updates, and it MUST NOT authorize
implementation or production apply directly.

#### CTR-V9-002 — Side-band non-interference
Creating a Collaboration Entry MUST NOT modify Context, execute a
Transition, create or modify a Node Visit or activation, change assignee
or current projections, increment `workflow_state_version`, create a
Workflow Event, consume `event_sequence`, or mutate any Assistance Case;
and MUST NOT cause any state command's expected-version check to fail.

#### CTR-V9-003 — Append-only immutability
Collaboration Entries MUST be immutable and retained forever, including
after cancel and archive; V1 provides no edit, delete, or void path
under any lifecycle state or authority.

#### CTR-V9-004 — Server-authored provenance and observation
Entry authorship MUST bind to the verified canonical Principal only;
`observed_node_visit_id` and `observed_workflow_state_version` MUST be
snapshotted by the server at append time; request bodies MUST NOT be
able to specify author, observed visit, observed version, or any kind.

#### CTR-V9-005 — Read authorization without payload escalation
Collaboration read access MUST follow the existing instance visibility
classification, with Historical Participants reading complete Entries;
canonical workflow facts surfaced alongside Entries MUST keep their own
payload read authority, and referencing a fact MUST NOT grant its
payload (`REFERENCE_DOES_NOT_GRANT_ACCESS`).

#### CTR-V9-006 — Write authorization
Entry appends MUST be limited to enabled Principals holding Domain
Owner, Creator, Current Assignee, or Historical Participant relation to
the Instance, and MUST fail closed on archived Instances;
TERMINAL-but-not-archived and CANCELLED-but-not-archived Instances MUST
remain writable. No string-role or new participant identity model MAY be
introduced.

#### CTR-V9-007 — Archive read-only
On archived Instances, Collaboration appends MUST fail closed and reads
MUST remain available under the visibility classification.

#### CTR-V9-008 — Single source of truth for formal facts
Formal workflow facts (RETURN reason, Submission payload, Assistance
payload and lifecycle, Events) MUST remain singly authored by their
existing authorities; the Collaboration Feed MUST project them directly
without duplication, and a failed or rolled-back command (including a
rejected RETURN) MUST NOT appear in the Collaboration space as a
committed fact.

#### CTR-V9-009 — Idempotent append
Entry creation MUST use client idempotency identity with canonical
request comparison: same key and same request replay the original
outcome; same key and different request conflict without mutation;
network-timeout retries MUST NOT produce duplicate Entries.

#### CTR-V9-010 — Exact inherited-clause replacement
Within the carried baseline below, this exception replaces exactly two
inherited meanings and no others: (a) the §5.6 authoritative-history
sentence is read as authoritative **workflow** history, and this
exception adds a separately governed, non-workflow-authoritative
side-band Collaboration fact family that is not workflow history and
never feeds workflow projections; (b) the §6 sentence granting
historical participants only "explicitly governed participation
history" gains this exception as that explicit governance for Instance
Collaboration read/write participation. Every other baseline obligation
carries forward unchanged.

#### CTR-V9-011 — Frozen V1 scope
No later Architecture, Spec, API, schema, migration, code, test, or UI
label may broaden this exception (kinds, reactions, mentions, search,
realtime transport, attachments, cross-instance threads, summary
authority, wake/dispatch triggers, or visibility escalation) without a
further lawful higher-authority change.

### 9. Acceptance criteria for the contracts

```text
ACC-V9-001 (CTR-V9-002, mechanical): after appending 100 Entries to an
  Instance, workflow_state_version and the Instance's workflow_events
  row count are unchanged.
ACC-V9-002 (CTR-V9-009): retrying an append with the same idempotency
  key and identical request yields exactly one Entry and replays the
  original response, including across process restart.
ACC-V9-003 (CTR-V9-004 + storage integrity): cross-Instance
  reply/reference attempts fail closed (HTTP-level aggregated 422 and
  storage-level composite-key enforcement); request bodies carrying
  author/kind/observed-* fields are rejected as unknown fields.
ACC-V9-004 (CTR-V9-005): a Historical Participant reads all Entries but
  does not gain Submission/Context payload it could not already read.
ACC-V9-005 (CTR-V9-006/007): appends succeed on
  TERMINAL-not-archived and CANCELLED-not-archived Instances and fail
  closed with a stable error code after archive.
ACC-V9-006 (CTR-V9-008): a rejected RETURN (422
  invalid_return_references) never appears in the Collaboration Feed;
  committed RETURN reasons in the Feed are byte-derived from the
  canonical Submission payload, not from any copied field.
```

### 10. Authoring handoff

The companion documents are `SVC_WORKFLOW_ARCHITECTURE_V0_4_2`
(architecture successor: side-band fact family, invariants, projection
semantics) and `SVC_WORKFLOW_INSTANCE_COLLABORATION_V1` (implementation
Spec: storage, wire, errors, tests). Both are proposed on the same
docs-only branch and share this candidate base. The Owner acceptance
packet must reference all three exact heads.

## Complete V8 baseline carried forward
...
Predecessor source: SVC_WORKFLOW_PRODUCT_BOUNDARY_V6 at e297ff1f3913133058d97bb30bcf8f63b3e137f9; file SHA256 1d162a6816360bff6829bdaee072ac9cc826f2e9e897a3b560ede9251540ec05.
