...
4. `.agents/protocol/SPEC_GOVERNANCE_V1.md` and `SPEC_FORMAT_V0.md` as needed;
...
After governance adoption is active, proposed governing Specs remain on docs-only PR branches. The `main` authority branch should contain only accepted or superseded governing Specs, except for an explicitly reviewed adoption transition while its PR is under review.

## Existing-authority bridge

Adoption is forward-only and does not bulk-migrate legacy documents.

- `PRODUCT-BOUNDARY.md` and its accepted whole-authority successors remain the repository Product Direction authority in their declared scope.
...
| `SVC_WORKFLOW_DEVELOPMENT_GOVERNANCE_ADOPTION_V2` | proposed | invariant | none | repository development process | `SVC_WORKFLOW_DEVELOPMENT_GOVERNANCE_ADOPTION_V1` |
...
While V2 is proposed, V1 remains the active local governance adoption.
The table is a discovery aid; each Spec's exact frontmatter and revision remain authoritative.
