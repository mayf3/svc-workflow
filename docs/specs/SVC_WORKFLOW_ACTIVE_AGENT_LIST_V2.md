...
date: 2026-09-16
candidate_base: 07d9117358113c89dc9bd4d483695c8d34b21efb
scope:
...
This is a docs-only whole-successor candidate for
`SVC_WORKFLOW_ACTIVE_AGENT_LIST_V1`. It changes no product semantics, source,
schema, deployment, role, grant, Workflow Instance, or production data. Its
Contracts remain inert while status is `proposed`. V1 remains the accepted
authority until an atomic V2 acceptance transaction. Implementation may begin
only after this exact successor is independently reviewed, Owner-accepted,
merged to `main`, and present as accepted authority in the implementation base.

V2 corrects only V1's exact implementation closure and the response-type
boundary needed to implement the already accepted contract lawfully. The V1
implementation attempt stopped before any product-code change after a red test
proved that the shared `DomainInstanceSummary` path cannot carry a global-only
field without changing the Domain-list wire contract.

```text
V1_IMPLEMENTATION_ATTEMPT=STOPPED_BEFORE_PRODUCT_CODE
RED_TEST_ESTABLISHED=YES
ROOT_CAUSE=SHARED_DOMAIN_INSTANCE_SUMMARY
MISSING_LOAD_BEARING_PATH=query_service.rs
V1_CLOSURE_LEGAL_IMPLEMENTATION=NO
PRODUCT_SEMANTIC_DELTA_FROM_V1=NONE
```
...
This six-path list is exact. `query_service.rs` is load-bearing because it maps
the global repository page into the HTTP-facing global summary. The response
type for the global route is `GlobalInstanceSummary`; the existing
`DomainInstanceSummary` remains byte-for-byte and behaviorally unchanged. No
implementation may put `current_executor_type` on `DomainInstanceSummary`.
...
SVC_WORKFLOW_PRODUCT_BOUNDARY_V8 (accepted product authority)
  -> SVC_WORKFLOW_ARCHITECTURE_V0_4_3 (accepted Architecture)
       -> SVC_WORKFLOW_ACTIVE_AGENT_LIST_V2 (this proposed whole successor)
...
authority and is not superseded. This Spec adds no reader. Architecture
V0_4_3 is already accepted. The dsh companion must pin the final accepted
svc-workflow V2 coordinate before its own acceptance.

If accepted, this Spec wholly supersedes V1 while preserving every V1 product
Decision, Contract, Acceptance obligation, and non-goal. The existing global
Reader Spec continues to own role gating; this Spec owns only the additive V8
query/projection contract and its lawful exact implementation closure.
...
For this global route, "summary" means the global-only
`GlobalInstanceSummary` produced through
`src/application/workflow_instance/query_service.rs`.
`DomainInstanceSummary` is outside this Contract and MUST remain unchanged in
shape and serialization behavior.
...
Also prove that Domain-list responses and `DomainInstanceSummary` serialization
are byte-for-byte unchanged while the global response uses
`GlobalInstanceSummary`.
...
NEXT_GATE=INDEPENDENT_SEMANTIC_REVIEW_OF_SVC_V2_AND_DSH_V4
```
