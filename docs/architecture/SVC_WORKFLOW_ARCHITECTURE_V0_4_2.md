---
authority_id: SVC_WORKFLOW_ARCHITECTURE_V0_4_2
status: proposed
...
date: 2026-09-15
revision: r1
change_name: WORKFLOW_INSTANCE_COLLABORATION_ARCHITECTURE_V0
...
# SVC_WORKFLOW_ARCHITECTURE_V0_4_2 — proposed instance-collaboration bounded successor
...
Predecessor source: SVC_WORKFLOW_ARCHITECTURE_V0_4_1 at ed99fa06a3067fe1d230699e9fed2b19542ab190; file SHA256 b996a6e7415201edc9c07ec27b1509ecc5e45dc09a5719589f9ebc90e5c600ca.

## Instance-collaboration exception: COLLAB-V0

This exception is governed by the Product Direction exception
`WORKFLOW_INSTANCE_COLLABORATION_AUTHORITY_V0`
(`SVC_WORKFLOW_PRODUCT_BOUNDARY_V9` §V9). It defines Architecture
semantics and invariants only. Consistent with the baseline §2.2
non-goals, this successor does not name endpoints, tables, columns,
indexes, DDL, or wire fields; those are owned by the accepted
implementation Spec (`SVC_WORKFLOW_INSTANCE_COLLABORATION_V1`) and its
contract bundle amendments.

### COLLAB-V0.1 — New side-band fact family

The architecture adds exactly one new durable fact family: the
per-Instance append-only Collaboration Entry family.

```text
Collaboration Entry is:
  an immutable, kind-free, instance-scoped side-band record
  authored by one verified canonical Principal
  carrying bounded free text
  optionally referencing, within the same Instance only:
    one earlier Collaboration Entry (reply)
    one Workflow Event
    one Submission
    one Assistance Case
  snapshotting, server-authored at append time:
    the Instance's current Node Visit identity
    the Instance's current workflow state version
  linked to exactly one completed command-receipt identity
```

It is a **non-workflow-state side-band family**. It is not a member of
the baseline §5.12 workflow/activation fact families, is not part of
event/replay/rebuild integrity, never feeds current Context/Visit/
activation/version projections, and is never a Scheduler input. The
existing four fact families plus activation facts remain the sole
workflow-authoritative history; the Collaboration Entry family records
participant collaboration only.

### COLLAB-V0.2 — Append command semantics

Entry creation is an idempotent command, not a state command.

```text
idempotency identity = authenticated principal + client idempotency key
request identity     = complete canonical request envelope hash
same key + same request  = replay original outcome
same key + new request   = conflict, zero mutation
append MUST NOT acquire the state-command serialization position of the
  Instance; it validates lifecycle and snapshots current Visit/version
  within its own transaction without asserting expected-version
deterministic failure = stable completed outcome (house rule)
```

The append transaction: verifies the authenticated Principal's
collaboration write relation; reads the Instance's current lifecycle,
current Visit identity, and current workflow state version; snapshots
those two observations into the new fact; validates every reference
against the same Instance; and commits the fact plus its receipt. No
`FOR UPDATE` serialization against concurrent state commands is
required or permitted for correctness of the side-band semantics; the
append MUST NOT increment `workflowStateVersion`, MUST NOT create a
Workflow Event, MUST NOT occupy `eventSequence`, MUST NOT create or
modify a Node Visit or activation, MUST NOT change assignee or current
projections, and MUST NOT mutate Assistance state. Conversely, state
commands are unaware of Collaboration Entries: no expected-version
check, event linkage, or projection rebuild accounts for them.

`observed_workflow_state_version` is an advisory observation record for
later readers ("what the author could see when writing"). It MUST NOT
be validated as an optimistic-concurrency input and MUST NOT gate any
command.

### COLLAB-V0.3 — Storage integrity discipline

Same-Instance referential integrity follows the existing composite-key
discipline: every cross-family reference (reply, Event, Submission,
Assistance Case) is keyed on `(referenced_id, workflow_instance_id)`
against the referenced family's per-Instance unique key, so a
cross-Instance reference is structurally impossible at the storage
level and fails closed. Entry rows are immutable after insert (no
update, no delete, no void, under any lifecycle state or administrative
authority in V1). Referenced facts are never copied into the Entry.

### COLLAB-V0.4 — Authorization binding

Collaboration read/write authorization binds to the existing instance
visibility classification and participation relations exactly as the
Product Direction exception defines them: any legal instance visibility
reads (Historical Participant reads complete Entries); Domain Owner,
Creator, Current Assignee, and Historical Participant write while the
Principal is enabled and the Instance is not archived;
TERMINAL-not-archived and CANCELLED-not-archived remain writable;
archived is read-only with appends failing closed. References never
grant payload access to the referenced fact. No new identity model,
string role, or global-role extension is introduced.

### COLLAB-V0.5 — Unified collaboration read projection

The Collaboration Feed is a non-authoritative, rebuildable read
projection merging, in one stable chronological order:

```text
COLLABORATION_ENTRY items  (the side-band facts)
WORKFLOW_FACT items        (minimal collaboration-relevant subset of
                            canonical facts, projected directly:
                            committed Submissions, RETURN facts,
                            Assistance lifecycle, Transition/terminal/
                            cancel facts)
```

Invariants: formal facts are projected from their canonical rows with
no second copy; payload visibility of each projected fact follows that
fact's own read authority; failed or rolled-back commands never appear;
Entries are visible to every Principal holding instance visibility;
feed ordering is a stable total order with keyset continuation; the
feed is a projection, not a second authority, and can be recomputed
from the fact families.

### COLLAB-V0.6 — Decisions

```text
DEC-COLLAB-001 — One new side-band fact family, architecture-separated
  from workflow/activation facts and replay integrity.
DEC-COLLAB-002 — Append is an idempotent non-state command; the
  state-command version/Event invariant is untouched and un-violated.
DEC-COLLAB-003 — Server-authored observation snapshot replaces any
  client-supplied visit/version claim.
DEC-COLLAB-004 — Kind-free V1 record; semantics live in body, reply,
  and references only.
DEC-COLLAB-005 — Archive is the single hard close point; terminal and
  cancelled Instances stay collaboration-writable.
DEC-COLLAB-006 — Unified feed is a bounded projection over canonical
  facts plus Entries, never a duplicate authority.
```

### COLLAB-V0.7 — Contracts

#### CTR-COLLAB-001 — Lifecycle and authority
V0_4_2 is inert until Owner acceptance and the atomic whole-authority
transition with V0_4_1 backlink and authority-map update; it authorizes
no code or production apply by itself and is governed by V9.

#### CTR-COLLAB-002 — Side-band separation
The Collaboration Entry family MUST remain outside the workflow/
activation fact families, outside event/replay/rebuild integrity, and
MUST never feed current projections, eligibility, dispatch, or wait
predicates.

#### CTR-COLLAB-003 — Append non-interference
Entry appends MUST NOT increment `workflowStateVersion`, create
Workflow Events, occupy `eventSequence`, create or modify Visits or
activations, change assignee or current projections, mutate Assistance,
or cause any state command's expected-version check to fail; state
commands MUST remain unaware of Entries.

#### CTR-COLLAB-004 — Server-authored provenance
Author identity MUST come only from verified authentication; observed
Visit and observed workflow state version MUST be server-snapshotted at
append; clients MUST NOT be able to supply author, kind, or observation
fields.

#### CTR-COLLAB-005 — Same-Instance referential integrity
Every Entry reference MUST resolve within the same Instance via the
composite-key discipline; cross-Instance references MUST be structurally
impossible at the storage level and MUST fail closed at the command
level with aggregated deterministic errors.

#### CTR-COLLAB-006 — Append idempotency
Appends MUST use principal/key identity with complete-request hashing;
same-key/same-request replays, same-key/changed-request conflicts with
zero mutation, and timeout retries never duplicate Entries.

#### CTR-COLLAB-007 — Unified feed projection
The feed MUST project canonical facts directly (no duplication), limit
WORKFLOW_FACT items to the minimal collaboration-relevant subset,
enforce each fact's own payload visibility, never surface failed or
rolled-back commands, and order all items in one stable total order
with keyset continuation.

#### CTR-COLLAB-008 — Authorization and archive boundary
Read follows instance visibility with complete Entries for Historical
Participants; write requires an enabled Principal holding one of the
four participation relations and a non-archived Instance; archive is
read-only fail-closed; references never grant referenced-fact payload
access.

#### CTR-COLLAB-009 — Naming and storage ownership
Concrete table, column, index, trigger, endpoint, DTO, error-code, and
pagination names are owned exclusively by the accepted implementation
Spec and its contract bundle; this Architecture pins only the semantics
and invariants above.

#### CTR-COLLAB-010 — No hidden expansion
Nothing beyond COLLAB-V0 scope (kinds, reactions, mentions, search,
realtime transport, attachments, cross-instance threads, AI summary
authority, wake/dispatch triggers, scheduler constructs, or visibility
escalation) may be added by any child authority without a further lawful
higher-authority change.

### COLLAB-V0.8 — Inherited-clause reconciliation

Within the carried baseline below, this exception reconciles exactly
three inherited meanings and changes no others: (a) §5.12's retained
immutable fact-family list describes workflow/activation-authoritative
families, and this successor adds the separately governed non-state
side-band family defined here; (b) CTR-ARCH-020's state-command
invariant applies to state commands, and Collaboration appends are
defined as non-state commands that still bind command-receipt identity;
(c) CTR-ARCH-040's no-hidden-expansion rule is satisfied by this
successor itself being the lawful higher-authority change recorded in
the Product Direction exception. The baseline's §2.2 non-goal "no
endpoint/table/DDL definitions" continues to hold for this document.

## Complete v0.4.1 baseline carried forward
