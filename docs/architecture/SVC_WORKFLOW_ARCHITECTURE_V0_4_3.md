...
# SVC_WORKFLOW_ARCHITECTURE_V0_4_3 — proposed Active Agent discovery successor

This is a whole-authority candidate. It contains the complete accepted
V0_4_1 predecessor below and adds exactly one Product V8 architecture
exception: HR V0 may use the existing global list for canonical
`ACTIVE + current executor AGENT` discovery and current-owner delivery.
All other V0_4_1 semantics remain in force. The predecessor remains accepted
until an independent semantic review, Owner exact-head acceptance, atomic
lifecycle transaction, final-head recheck, and merge.

Predecessor source:
`SVC_WORKFLOW_ARCHITECTURE_V0_4_1@ed99fa06a3067fe1d230699e9fed2b19542ab190`;
SHA-256 `b996a6e7415201edc9c07ec27b1509ecc5e45dc09a5719589f9ebc90e5c600ca`.

## Concurrent Architecture candidate gate

A separate unmerged remote branch
`docs/instance-collaboration-authority-v1@183c85c8766e6fc0b3451820f0be7fda16b4afe6` currently proposes
`SVC_WORKFLOW_ARCHITECTURE_V0_4_2`, also as a whole successor of V0_4_1.
It is not accepted authority and is not modified by this candidate.

Exactly one acceptance order is lawful:

1. if V0_4_2 is accepted first, this candidate must be regenerated as a
   byte-complete successor of accepted V0_4_2, preserving its collaboration
   exception, before review/acceptance;
2. if this V0_4_3 is accepted first, the collaboration candidate must be
   regenerated and renumbered as the next whole successor of V0_4_3.

No review or Owner approval of one candidate accepts, composes, supersedes, or
disposes the other. Silent partial composition is forbidden.

## Active-Agent discovery exception: AAD-V0

### AAD-V0.1 — Canonical current-executor relation

For the global list only, current executor type is derived from the exact
current Visit assignee Principal in the same read snapshot:

```text
current Instance
  -> exact current Node Visit
  -> exact assignee Principal
  -> principal_type HUMAN | AGENT
```

This relation is owned by svc-workflow. It is not inferred from Agent session,
display name, external reference, title, metadata, activation, eligibility,
execution class, node key, or external runtime state. SERVICE is not an
executor value. A Terminal node has no current executor.

### AAD-V0.2 — Product ACTIVE and exact HR V0 predicate

For this exception, product ACTIVE is exactly:

```text
cancelled = false
AND archived_at IS NULL
AND current node type is not TERMINAL
```

The split wire expression is exactly:
...
The existing `lifecycle` and `status` parameters keep their established
independent meanings and defaults. No caller may treat `lifecycle=active`
alone as the dispatch pool.

### AAD-V0.3 — Bounded compatibility exception

V0_4_1 §5.11 and §5.14 continue to make activation-driven delivery the normal
new-model Scheduler path. Product V8 creates one bounded exception: HR V0 may
use the exact AAD-V0.2 global-list result as normal work discovery and may send
only to the exact current owner resolved from that result.

This exception does not make the global list an Activation or Dispatch Intent
feed, does not change activation facts, and does not let any other Scheduler or
Dispatcher scan Workflow Instances for normal work.

### AAD-V0.4 — No hidden dispatchability classifier

The list predicate contains only product ACTIVE plus exact current executor
type. Identity health, runtime presence, Domain enabled state, Definition
future-chain health, test/canary status, quarantine, owner-required disposition,
visibility inference, title/content, activation, eligibility, and external side
effects are not list filters.

An ACTIVE+AGENT row with an invalid current owner is a data or Definition
invariant violation. It remains visible until repaired or lawfully removed from
ACTIVE. Failed exact owner resolution never authorizes a fallback identity or a
send.

### AAD-V0.5 — Single delivery authority mode

Activation-driven normal delivery and HR V0 list-driven delivery are two
mutually exclusive production delivery-authority modes for one environment.
They must not concurrently consume work.

Activating HR V0 requires durable operational evidence that activation-driven
normal delivery is stopped and cannot admit new sends for that environment.
Activating activation-driven delivery requires HR V0 normal delivery to be
stopped. Mode transition is serialized through the existing production
mutation/runtime lane and independently authorized; this Architecture adds no
Workflow field, lease, claim, tombstone, consumption key, dispatch ledger, or
retry engine.

Read-only diagnostic use of either surface may coexist. Only delivery authority
is exclusive. An outcome-unknown send is never replayed under the other mode.

### AAD-V0.6 — Exact current-owner delivery boundary

For each returned row, HR passes
`current_assignee_principal_id` to the accepted exact Principal-to-Agent
resolver. A successful canonical enabled Agent resolution may be delivered
through the accepted Agent Session Messaging path using trusted
`workflow_execution` origin metadata bound to the exact
`workflowInstanceId` and `nodeVisitId`. The executor acts with its own
credential and grants.

This Architecture adds no transition authority. HR does not impersonate the
executor, fabricate completion, or use the source Agent credential in the
target run.

### AAD-V0.7 — Production data invariant

Before HR V0 delivery activation, the complete paginated ACTIVE+AGENT set must
equal the independently frozen canonical dispatchable-Agent ledger:
...
Invalid/historical rows must be repaired, normalized, cancelled, archived, or
terminalized under their own accepted authorities. Adding a query filter to
make the counts pass is forbidden.

Post-snapshot new ACTIVE rows are audited separately and cannot be silently
added to an earlier frozen denominator.

### AAD-V0.8 — Authority and gate separation

Architecture acceptance grants no implementation or production authority.
Required downstream authorities are:

1. accepted svc-workflow implementation Spec for the canonical projection and
   server-side filter;
2. accepted dsh-agent-core Broker companion for transparent pass-through;
3. independently reviewed implementation heads;
4. separate merge, deployment, cleanup, data-invariant, runtime-mode, and real
   dispatch gates.

## AAD-V0 contracts

### CTR-AAD-001 — Same-snapshot canonical derivation

The server derives nullable `current_executor_type` only from the exact current
Visit assignee Principal within the list snapshot. Its only non-null values are
HUMAN and AGENT.

### CTR-AAD-002 — Exact query composition

HR V0 uses all three filters from AAD-V0.2. Missing any one of them is not an
equivalent query.

### CTR-AAD-003 — Transparent Broker

The Broker may expose and forward `currentExecutorType` and return
`current_executor_type` verbatim. It does not derive, filter, classify,
resolve, cache, or map executor semantics.

### CTR-AAD-004 — No local compatibility authority

Session IDs, display names, legacy external references, canonical-agent
successor enrichment, or historical assignee IDs never become business
ownership authority.

### CTR-AAD-005 — Invariant defects stay visible

Server projection is not permitted to suppress an ACTIVE AGENT merely because
external identity resolution, runtime validation, Domain state, or future-chain
validation fails.

### CTR-AAD-006 — Exclusive delivery modes

No environment may have activation-driven normal delivery and HR V0 list-driven
normal delivery active concurrently. Exact activation/stop evidence and the
shared runtime lane are acceptance requirements, not list filters.

### CTR-AAD-007 — Exact-set production acceptance

Production activation requires complete, cursor-exhausted API and canonical DB
sets, a frozen denominator plus separately recorded new rows, and exact
zero-difference comparison.

### CTR-AAD-008 — No product/data mutation

This candidate performs no data cleanup, Definition repair, identity migration,
Workflow transition, role/grant change, deployment, or runtime mode switch.

## AAD-V0 acceptance

### ACC-AAD-001 — Query truth table

Required evidence covers HUMAN, AGENT, cancelled, archived, terminal, invalid
enum, cursor boundaries, and same-snapshot concurrent-change behavior. The
three-filter result equals the DB relation exactly.

### ACC-AAD-002 — No inference

Static and behavioral evidence proves no title, metadata, session, display
name, external reference, activation, eligibility, execution-class, or external
identity source participates in executor derivation.

### ACC-AAD-003 — Compatibility

With the new parameter absent, existing list authorization, fields, ordering,
pagination, lifecycle/status semantics, error envelope, and diagnostic behavior
remain compatible.

### ACC-AAD-004 — Broker equality

Wire capture proves the Broker forwards the input and output without local
classification or semantic transformation.

### ACC-AAD-005 — False-positive/negative closure

After separately authorized instance cleanup/repair, a complete real HR query
and independent DB ledger comparison prove both false counts are zero. One
fresh legal Agent-owned instance then completes HR fetch, exact owner
resolution, one accepted send, target execution, transition, and persisted
readback under separate production authority.

### ACC-AAD-006 — Delivery-mode exclusion

Operational evidence proves only one normal delivery authority is active, the
other cannot admit sends, and an outcome-unknown receipt is not replayed across
the mode boundary.

### ACC-AAD-007 — Concurrent-candidate lifecycle safety

Fresh branch/authority census proves the candidate is a whole successor of the
currently accepted Architecture and preserves every separately accepted
intervening exception. Any V0_4_2/V0_4_3 ordering drift stops acceptance.

## AAD-V0 rejected alternatives

- Client-side HUMAN/AGENT inference or filtering: duplicates svc-workflow
  semantics and creates drift.
- A stored executor field or compatibility mapping: introduces a second
  authority.
- A broader dispatchability/readiness classifier: exceeds Product V8.
- Concurrent activation and HR list dispatch: permits duplicate delivery.
- Filtering invalid owners from discovery: hides invariant violations.
- Reusing the historical V5-bound dispatchability proposal: stale and expressly
  rejected by Product V8.
- Treating an Architecture or Spec acceptance as cleanup/deploy authority:
  violates gate separation.

## Complete accepted V0_4_1 baseline restatement
